...
const Testimonials = () => (
  ...
  justify-center items-center w-full feedback-container relative z-[1]">
    ...
    <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 relative z-[1] mt-10">
      {[file1, file2, file3, file4, file5, file6].map((img, index) => (
        <div key={index} className='w-full h-full flex justify-center items-center p-2 hover:scale-105 transition-transform duration-300'>
          <img src={img} alt={`project-${index}`} className="w-full h-[300px] object-cover rounded-[20px] shadow-lg" />
        </div>
      ))}
    </div>
